...
export async function checkForUpdates(verbose = false): Promise<void> {
  try {
    ...
    if (!response.ok) {
      if (verbose) console.log(pc.dim('⚠ Could not check for updates (GitHub API unavailable)'));
      return;
    }
    ...
    if (!latestCommit) {
      if (verbose) console.log(pc.dim('⚠ Could not parse latest commit from GitHub'));
      return;
    }
    ...
    if (currentVersion && currentVersion === latestCommit) {
      if (verbose) console.log(pc.dim(`✓ Bridge is up to date (${latestCommit})`));
      return;
    }
    ...
    try {
      ...
      if (verbose) console.log(pc.dim('✓ Cleared npx cache'));
    } catch (err) {
      if (verbose) console.log(pc.dim('⚠ Could not clear npx cache (may already be clean)'));
    }
